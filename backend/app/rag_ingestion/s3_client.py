...
class S3IndexClient:
    ...
    def _retry(self, fn, description: str):
        ...
        raise RuntimeError(f"S3 operation failed after retries: {description}") from last_error

    def upload_index_bundle(self, faiss_path: Path, metadata_path: Path):
        self.ensure_layout()
        self._retry(
            lambda: self.s3.upload_file(str(faiss_path), self.bucket, "indexes/faiss.index"),
            "upload faiss.index",
        )
        self._retry(
            lambda: self.s3.upload_file(str(metadata_path), self.bucket, "indexes/metadata.parquet"),
    ...
    def download_index_bundle(self, faiss_path: Path, metadata_path: Path):
        ...
        self._retry(
            lambda: self.s3.download_file(self.bucket, "indexes/faiss.index", str(faiss_path)),
