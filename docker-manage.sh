...
show_help() {
    ...
    echo ""
    echo "Environment:"
    echo "  .env.sample - Configuration file (edit values directly)"
    echo ""
    echo "Examples:"
...
# Function to ensure .env.sample file exists
ensure_env_file() {
    if [[ ! -f ".env.sample" ]]; then
        echo "❌ .env.sample file not found"
        echo "   Please create .env.sample configuration file"
        exit 1
    fi
}
...
aws_setup() {
    ...
    echo "==========================="
    echo ""
    echo "📋 Configuration File: .env.sample"
    echo ""
    
    if [[ -f ".env.sample" ]]; then
        source .env.sample 2>/dev/null || true
