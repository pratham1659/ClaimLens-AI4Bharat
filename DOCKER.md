...
```bash
# Edit .env.sample for your configuration (optional for local development)
nano .env.sample
```
...
### Single Configuration File: `.env.sample`

Edit `.env.sample` directly for your configuration:
...
1. Configure `.env.sample`:
   ```bash
   # Edit .env.sample - set ENVIRONMENT=production and AWS credentials
   nano .env.sample
   ```
